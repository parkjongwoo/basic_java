...
public class Ex11_arrange_quick {
	...
	/**
	 * -좌측 검색위치는 left인덱스부터 pivot인덱스까지 이동하며 pivot의 값보다 큰 값을 검색
	 * -우측 검색위치는 right인덱스부터 pivot인덱스까지 이동하며 pivot의 값보다 작은 값을 검색
	 * -좌측 검색 > 우측 검색 순으로 검색
	 * -좌측 검색값을 찾으면 멈추고 우측 검색 시작
	 *  이후 우측 검색값도 찾으면 좌우측 값을 교환 후 좌측부터 다시 시작
	 *  좌측 검색위치가 우측검색위치와 만나면 pivot과 값비교 후 pivot값 교환삽입
	 *  pivot기준으로 분할 완료.
	 *  다음 회차는 left~pivot-1, pivot+1~right 두 범위를 분할하도록 재귀호출
	 *  left==right인 경우 정렬 종료.
	 * @param left 이번 회차의 첫번째 배열 요소 인덱스
	 * @param right 이번 회차의 마지막 배열 요소 인덱스
	 * @param pivot 피봇 인덱스
	 * @param arr 정렬할 배열
	 */
	public static void quickArrange(int left, int right, int pivot, int[] arr) {
		if(left==right) return;//좌우 검색위치가 동일한 경우 값이 1개이므로 정렬불가 바로 종료
